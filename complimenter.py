...
class Window(Frame):
    ...
    def init_window(self):
        ...
        createCompliment = Button(self, text="Generate Compliment", fg="green", command=self.compliment_generator)
        createCompliment.place(x=50, y=100)

        # Generate Insult button and it's attributes
        createInsult = Button(self, text="Generate Insult", fg="red", command=self.insult_generator)
        createInsult.place(x=450, y=100)

        # labels display the compliment (or insult) with each button click
        # self.root = tk.Tk()
        self.textOne = tk.StringVar()
        self.textTwo = tk.StringVar()
        self.textOne.set(" ")
        self.textTwo.set(" ")

        # label displaying the generated compliments
        self.displayCompliment = Label(self, textvariable=self.textOne, fg="green")
        self.displayCompliment.pack()
        self.displayCompliment.place(x=50, y=150)

        # label displaying the generated insults
        self.displayInsult = Label(self, textvariable=self.textTwo, fg="red")
        self.displayInsult.pack()
        self.displayInsult.place(x=450, y=150)

        # the Quit button and it's attributes
        quitButton = Button(self, text="Quit Program", command=self.client_exit)
        quitButton.place(x=335, y=250)

    # function to create the randomly generated compliments
    def compliment_generator(self):

        self.displayCompliment.config(text=" ")

        # first list of compliment words
        list_one = ['airy','amorous','balmy','bespiced','beteeming','blazoning','bonny','brisky','budding','candied',
                    'celestial','chafeless','choicely','courtly','dainty','daisied','damasked','enchanted','engilded',
                    'fettled','honeysuckle','jovial','leavened','mannerly','marbled','meek','nonpareil','orbed','palmy',
                    'posied','replenished','sightly','silken','sovereign','sphery','sterling','sturdy','taffeta',
                    'tenderful','virtuous','worthy']
        ...
        list_two = ['all-hallown','alms-deed','brother-love','burley-boned','cheek-roses','crow-feathered',
                    ...
                    'smoothy-pated','softly-sprighted','sweet-seasoned','thrice-crowned','tiger-footed','top-gallant',
                    'truest-mannered','weeping-ripe','well-breathed','well-flavoured','young-eyed']
        ...
        list_three = ['aglet-baby','argosy','bodykins','bona-roba','bullyrook','chuck','coach-fellow','crystal-button',
                      'cuckoo-bud','dewberry','eglantine','esquire','ear-kissing','flax-wench','gamester','handy-dandy',
                      'heartling','homage','juvenal','kicksy-wicksy','kid-fox','lambkin','lodestar','madonna',
                      'minstrel','nicety','pew-fellow','pittikins','prizer','primrose','rarity','ringlet',
                      'shoulder-clapper','tender-smelling','thunder-maker','time-pleaser','turtle-dove','wafer-cake',
                      'whiffler','wit-snapper','velvet-guard']

        # random number to choose compliment from third list
        compliment_three = list_three[random.randint(0, len(list_three)-1)]

        self.textOne.set("Thou " + compliment_one + " " + compliment_two + " " + compliment_three)

    # function to create the randomly generated insults
    def insult_generator(self):

        self.displayInsult.config(text=" ")

        # first list of insult words
        list_one = ['bawdy','brainsick','fat','false','foul','greasy','infectious','kindless','lecherous','paunchy',
                    'puking','qualling','remorseless','rotten','sanguine','spherical','starveling','vile','venomous',
                    'ugly','yeasty']
        ...
        list_three = ['bed-presser','biscuit','boar-pig','bull-pizzle','carbuncle','coward','death-token','eel-skin',
                      'fustilarian','harlot','hog','horseback-breaker','knave','madman','neat-tongue','ratsbane',
                      'rogue','stock-fish','whoreson','villain','worms-meat']

        # random number to choose insult from third list
        insult_three = list_three[random.randint(0, len(list_three) - 1)]

        self.textTwo.set("Thou " + insult_one + " " + insult_two + " " + insult_three)
